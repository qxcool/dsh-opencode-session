...
// Provider routes OpenCode(Go) requests are served under. The wire-protocol
// split serves these under four route keys (settings.yaml): the pi-ai catalog
// ids plus the split-out completions/anthropic routes. A route naming an
// installed pi-ai catalog provider keeps that provider's id as its route key,
// so all four catalog ids are covered; users who route OpenCode through a
// custom provider key add it through config.
const DEFAULT_PROVIDERS = ['opencode', 'opencode-go', 'opencode-go-responses', 'opencode-go-anthropic']
